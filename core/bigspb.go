...
func NewBigSPB(corer *BigCore, Proposer NodeID, View int) *BigSPB {
	instance := &BigSPB{
		corer:    corer,
		Proposer: Proposer,
		View:     View,
...
func (s *BigSPB) processProposeMsg(p *ProposeMsg) {
...
func (s *BigSPB) processVoteMsg(v *VoteMsg) {
	s.hMutex.Lock()
	...
	if nums == int32(s.corer.committee.HightThreshold()) { //2f+1?
		...
		s.hMutex.Unlock()
		if v.Height == HEIGHT_1 && s.Proposer == s.corer.nodeID {
			//new propose
...
func (s *BigSPB) processSpeedVoteMsg(sv *SpeedVoteMsg) {
...
func (s *BigSPB) SpeedCert() bool {
	return s.isSpeedCert.Load()
}

func (s *BigSPB) DecCert() bool {
	s.hMutex.Lock()
	defer s.hMutex.Unlock()
	return s.blockQcs[HEIGHT_1] != nil && s.blockQcs[HEIGHT_2] != nil
}

func (s *BigSPB) BlockHash(height uint8) crypto.Digest {
	s.hMutex.Lock()
	defer s.hMutex.Unlock()
	if height > HEIGHT_2 {
...
func (s *BigSPB) QCLEvel() (uint8, *BlockQC) {
	s.hMutex.Lock()
	defer s.hMutex.Unlock()
